class Item < ActiveRecord::Base
  ...
  def clone_with_attachments
    ...
    self.documents.each do |doc|
      doc_clone = doc.clone
      doc_clone.save
      unless doc.base_document_type?
        doc_clone.update_attribute(:document, File.open(doc.document.path))
      end
      new_clone.documents << doc_clone
    end
    self.model_translations.each do |trl|
      trl_clone = trl.clone
      trl_clone.translateable_id = new_clone.id
      trl_clone.save
    end

    self.standards.each {|s| new_clone.standards << s}
    self.complectations.each {|i| new_clone.complectations << i}
