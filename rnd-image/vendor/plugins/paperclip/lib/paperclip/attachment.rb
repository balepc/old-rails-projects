...
module Paperclip
  ...
  class Attachment
    ...
    def initialize name, instance, options = {}
      ...
      @lazy_styles       = options[:lazy_styles]
      @upload_module_identify = options[:upload_module_identify]
    ...
    def styles_processed?
      @styles.keys.each do |style|
        return false unless style_processed?(style)
      end
      true
    end

    def process_styles!
      post_process_styles(true)
    end
    ...
    def post_process(force_lazy=false) #:nodoc:
      return if @queued_for_write[:original].nil?
      solidify_style_definitions
      return if fire_events(:before)
      post_process_styles(force_lazy)
    ...
    def post_process_styles(force_lazy=false) #:nodoc:
      @styles.each do |name, args|
        begin
          next unless process_style?(force_lazy, name, args)
    ...
    def process_style?(force_lazy, name, args)
      return true unless @lazy_styles
      if force_lazy
        !style_processed?(name)
      else
        args[:process_force]
      end
    end

    def style_processed?(name)
